 ...
 *support text parameter REDESIGNED: GuiTextInputBox() to support password 3.1
 *(12-Jan-2022) REVIEWED: Default style for consistency (aligned with rGuiLayout
 *v2.5 tool) REVIEWED: GuiLoadStyle() to support compressed font atlas image
 *data and unload previous textures REVIEWED: External icons usage logic
 *                         REVIEWED: GuiLine() for centered alignment when
 *including text RENAMED: Multiple controls properties definitions to prepend
 *RAYGUI_ RENAMED: RICON_ references to RAYGUI_ICON_ for library consistency
...
RAYGUIAPI int GuiTextInputBox(
    ...
    int *secretViewActive);  // Text Input Box control, ask for text, supports
                             // secret
RAYGUIAPI Color
...
RAYGUIAPI void GuiSetIconScale(
    unsigned int scale);  // Set icon scale (1 by default)
