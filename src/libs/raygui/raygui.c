...
static unsigned int
    guiIcons[RAYGUI_ICON_MAX_ICONS * RAYGUI_ICON_DATA_ELEMENTS] = {
        ...
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_206
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_207
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_208
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_209
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_210
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_211
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_212
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_213
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_214
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_215
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_216
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_217
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_218
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
        0x00000000, 0x00000000, 0x00000000,  // ICON_219
        0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
...
static int GetCharPressed(
    void);  // -- GuiTextBox(), GuiTextBoxMulti(), GuiValueBox()
//-------------------------------------------------------------------------------
...
static void SetShapesTexture(Texture2D tex,
                             Rectangle rec);                // -- GuiLoadStyle()
...
static Vector2 MeasureTextEx(
    Font font, const char *text, float fontSize,
    float spacing);  // -- GetTextWidth(), GuiTextBoxMulti()
static void DrawTextEx(Font font, const char *text, Vector2 position,
...
static void GuiDrawRectangle(
    Rectangle rec, int borderWidth, Color borderColor,
...
static Vector3 ConvertHSVtoRGB(
    Vector3 hsv);  // Convert color data from HSV to RGB
static Vector3 ConvertRGBtoHSV(
    Vector3 rgb);  // Convert color data from RGB to HSV

static int GuiScrollBar(
    Rectangle bounds, int value, int minValue,
...
Rectangle GuiScrollPanel(Rectangle bounds, const char *text, Rectangle content,
  ...
  if ((state != STATE_DISABLED) && !guiLocked) {
    ...
    if (CheckCollisionPointRec(mousePoint, bounds)) {
      ...
      if (hasHorizontalScrollBar &&
          (IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_SHIFT)))
...
bool GuiLabelButton(Rectangle bounds, const char *text) {
  ...
  float textWidth =
      MeasureTextEx(guiFont, text, (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
...
bool GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode) {
  ...
  int textAlignment = editMode && textWidth >= textBounds.width
                          ? TEXT_ALIGN_RIGHT
  ...
  if ((state != STATE_DISABLED) && !guiLocked) {
    ...
    if (editMode) {
      ...
      if ((keyCount + byteSize) < textSize) {
        // float maxWidth = (bounds.width - (GuiGetStyle(TEXTBOX,
        // TEXT_INNER_PADDING)*2));
      ...
      if (textAlignment == TEXT_ALIGN_CENTER)
        ...
        cursor.x = bounds.x + bounds.width -
                   GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING) -
  ...
  // in case we edit and text does not fit in the textbox show right aligned and
  // character clipped, slower but working
...
#define RAYGUI_VALUEBOX_MAX_CHARS 32
  ...
  if (editMode) {
    ...
    Rectangle cursor = {
        bounds.x + GetTextWidth(textValue) / 2 + bounds.width / 2 + 2,
...
bool GuiTextBoxMulti(Rectangle bounds, char *text, int textSize,
                     bool editMode) {
  GuiState state = guiState;
  bool pressed = false;
  ...
  Rectangle textAreaBounds = {
      ...
      bounds.y + GuiGetStyle(TEXTBOX, BORDER_WIDTH) +
          GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING),
      bounds.width - 2 * (GuiGetStyle(TEXTBOX, BORDER_WIDTH) +
                          GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING)),
      bounds.height - 2 * (GuiGetStyle(TEXTBOX, BORDER_WIDTH) +
                           GuiGetStyle(TEXTBOX, TEXT_INNER_PADDING))};
      ...
      if (textLength > 0) {
        if (IsKeyPressed(KEY_BACKSPACE)) {
          if ((unsigned char)text[textLength - 1] < 127) {
            ...
            int charUTF8Length = 0;
            while ((charUTF8Length < textLength) &&
                   ((unsigned char)text[textLength - 1 - charUTF8Length] &
                    0b01000000) == 0)
              charUTF8Length++;
  ...
  for (int i = 0, codepointLength = 0; text[i] != '\0'; i += codepointLength) {
    ...
    if ((codepointLength == 1) && (codepoint == '\n')) {
      cursorPos.y += (guiFont.baseSize * scaleFactor +
                      GuiGetStyle(TEXTBOX, TEXT_LINES_SPACING));  // Line feed
      ...
      DrawTextCodepoint(
          guiFont, codepoint, cursorPos, (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
          Fade(GetColor(GuiGetStyle(TEXTBOX, TEXT + (state * 3))), guiAlpha));
      ...
      cursorPos.x += (glyphWidth * scaleFactor +
                      (float)GuiGetStyle(DEFAULT, TEXT_SPACING));
...
#define RAYGUI_MESSAGEBOX_BUTTON_PADDING 12
  ...
  Vector2 textSize = MeasureTextEx(guiFont, message,
                                   (float)GuiGetStyle(DEFAULT, TEXT_SIZE), 1);
...
#define RAYGUI_TEXTINPUTBOX_HEIGHT 28
  ...
  if (message != NULL) {
    ...
    textBounds.x = bounds.x + bounds.width / 2 - textSize.x / 2;
    textBounds.y = bounds.y + RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT +
                   messageInputHeight / 4 - textSize.y / 2;
    textBounds.width = textSize.x;
  ...
  if (secretViewActive != NULL) {
    ...
    *secretViewActive = GuiToggle(
        RAYGUI_CLITERAL(Rectangle){
            textBoxBounds.x + textBoxBounds.width - RAYGUI_TEXTINPUTBOX_HEIGHT,
          ...
          case 'f': {
            ...
            if (charmapFileName[0] != '0') {
              ...
              if (charValues != NULL) {
                ...
                font =
                    LoadFontEx(TextFormat("%s/%s", GetDirectoryPath(fileName),
                                          fontFileName),
      ...
      if (fontDataSize > 0) {
        ...
        if (fontImageCompSize < fontImageUncompSize) {
          ...
          if (dataUncompSize != fontImageUncompSize)
            RAYGUI_LOG(
                "WARNING: Uncompressed font atlas image data could be "
                "corrupted");
...
char **GuiLoadIcons(const char *fileName, bool loadIconsName) {
  ...
  if (rgiFile != NULL) {
    ...
    if ((signature[0] == 'r') && (signature[1] == 'G') &&
      ...
      fread(guiIcons, iconCount * (iconSize * iconSize / 32),
            sizeof(unsigned int), rgiFile);
    }
...
// Get icon bit data
// NOTE: Bit data array grouped as unsigned int
// (RAYGUI_ICON_SIZE*RAYGUI_ICON_SIZE/32 elements)
unsigned int *GuiGetIconData(int iconId) {
  ...
  if (iconId < RAYGUI_ICON_MAX_ICONS)
    memcpy(iconData, &guiIcons[iconId * RAYGUI_ICON_DATA_ELEMENTS],
           RAYGUI_ICON_DATA_ELEMENTS * sizeof(unsigned int));

  return iconData;
}

// Set icon bit data
// NOTE: Data must be provided as unsigned int array
// (RAYGUI_ICON_SIZE*RAYGUI_ICON_SIZE/32 elements)
void GuiSetIconData(int iconId, unsigned int *data) {
  if (iconId < RAYGUI_ICON_MAX_ICONS)
    memcpy(&guiIcons[iconId * RAYGUI_ICON_DATA_ELEMENTS], data,
           RAYGUI_ICON_DATA_ELEMENTS * sizeof(unsigned int));
}
...
#define BIT_SET(a, b) ((a) |= (1u << (b)))
  ...
  // data element
  BIT_SET(guiIcons[iconId * RAYGUI_ICON_DATA_ELEMENTS +
                   y / (sizeof(unsigned int) * 8 / RAYGUI_ICON_SIZE)],
          x + (y % (sizeof(unsigned int) * 8 / RAYGUI_ICON_SIZE) *
               RAYGUI_ICON_SIZE));
}
...
#define ICON_TEXT_PADDING 4
  ...
  if ((text != NULL) && (text[0] != '\0')) {
    ...
    size = MeasureTextEx(guiFont, text + textIconOffset, fontSize,
                         (float)GuiGetStyle(DEFAULT, TEXT_SPACING));
...
static Rectangle GetTextBounds(int control, Rectangle bounds) {
  ...
  switch (control) {
    case COMBOBOX:
      bounds.width -= (GuiGetStyle(control, COMBO_BUTTON_WIDTH) +
                       GuiGetStyle(control, COMBO_BUTTON_SPACING));
      break;
    case VALUEBOX:
      break;  // NOTE: ValueBox text value always centered, text padding applies
              // to label
    default: {
...
#define ICON_TEXT_PADDING 4
  ...
  if ((text != NULL) && (text[0] != '\0')) {
    ...
    // TODO: REVIEW: We consider text size in case of line breaks! ->
    // MeasureTextEx() depends on raylib!
    Vector2 textSize =
        MeasureTextEx(GuiGetFont(), text, GuiGetStyle(DEFAULT, TEXT_SIZE),
    ...
    switch (alignment) {
      case TEXT_ALIGN_LEFT: {
        position.x = bounds.x;
        position.y = bounds.y + bounds.height / 2 - textSize.y / 2 +
...
#endif
    DrawTextEx(guiFont, text, position, (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
               (float)GuiGetStyle(DEFAULT, TEXT_SPACING), tint);
    //---------------------------------------------------------------------------------
  }
}
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue,
  ...
  if (isVertical) {
    ...
    slider = RAYGUI_CLITERAL(Rectangle){
        ...
        (float)scrollbar.y + (int)(((float)(value - minValue) / range) *
    ...
    sliderSize = (sliderSize >= scrollbar.width)
                     ...
                     : sliderSize;  // Make sure the slider won't get outside of
                                    // the scrollbar
    slider = RAYGUI_CLITERAL(Rectangle){
        (float)scrollbar.x + (int)(((float)(value - minValue) / range) *
      ...
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        ...
        if (!isVertical) {
          Rectangle scrollArea = {
              arrowUpLeft.x + arrowUpLeft.width, arrowUpLeft.y, scrollbar.width,
              bounds.height - 2 * GuiGetStyle(SCROLLBAR, BORDER_WIDTH)};
          if (CheckCollisionPointRec(mousePoint, scrollArea))
            value =
                (int)(((float)(mousePoint.x - scrollArea.x - slider.width / 2) *
                       range) /
                          (scrollArea.width - slider.width) +
                      minValue);
        } else {
          Rectangle scrollArea = {
              arrowUpLeft.x, arrowUpLeft.y + arrowUpLeft.height,
              bounds.width - 2 * GuiGetStyle(SCROLLBAR, BORDER_WIDTH),
              scrollbar.height};
          if (CheckCollisionPointRec(mousePoint, scrollArea))
            value = (int)(((float)(mousePoint.y - scrollArea.y -
                                   slider.height / 2) *
                           range) /
