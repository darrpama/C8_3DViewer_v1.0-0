...
void drawHelpInfo() {
  ...
  DrawText("- Alt + Ctrl + Mouse Wheel Pressed for Smooth Zoom", 40, 200, 10,
           DARKGRAY);
...
void drawCameraInfo(App *app) {
  ...
  char position[256];
  sprintf(position, " x:%f; y:%f; z:%f", app->scene.camera.position.x,
          app->scene.camera.position.y, app->scene.camera.position.z);
  DrawText(position, 40, 250 + 40, 12, DARKGRAY);

  DrawText("Camera target", 30, 250 + 60, 14, BLACK);
  char target[256];
  sprintf(target, " x:%f; y:%f; z:%f", app->scene.camera.target.x,
          app->scene.camera.target.y, app->scene.camera.target.z);
  DrawText(target, 40, 250 + 80, 12, DARKGRAY);

  DrawText("Camera up", 30, 250 + 100, 14, DARKGRAY);
  char up[256];
  sprintf(up, " x:%f; y:%f; z:%f", app->scene.camera.up.x,
          app->scene.camera.up.y, app->scene.camera.up.z);
...
void drawFileInfo(App *app) {
  ...
  DrawText(basename(app->ui.uploadBtn.fileNameToLoad), 170, top + 28, 14,
           BLACK);
